...
void mesh_skinned::apply_skinning(skeleton_geometry const& skeleton)
{
    ...
    for(int k_vertex=0 ; k_vertex<N_vertex ; ++k_vertex)
    {
        ...
        //  vertex(k_vertex) = ...
        /*int j = vertex_weight_data[0].joint_id;
        float w = vertex_weight_data[0].weight;

        vertex_data[k_vertex] = vertex_weight_data.weight * (skeleton[j].orientation * vertices_original_data[k_vertex]
                                + skeleton[j].position);*/


        int nb_j = vertex_weight_data[k_vertex].size();
        int j;
        float w;
        vertex_data[k_vertex]=vec3();
        for(int i=0 ; i<nb_j ; i++)
        {
            j = vertex_weight_data[k_vertex][i].joint_id;
            w = vertex_weight_data[k_vertex][i].weight;
            vertex_data[k_vertex] += w * (skeleton[j].orientation * vertices_original_data[k_vertex]
                                            + skeleton[j].position);
        }
