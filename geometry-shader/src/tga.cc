...
{
  unsigned char *alt = buffer + (info->bytes * (info->width - 1));

  for (int x = 0; x * 2 <= info->width; x++)
  {
    for (int s = 0; s < info->bytes; s++)
    {
      unsigned char temp = buffer[s];
      buffer[s] = alt[s];
      alt[s] = temp;
    }
 ...
 * upsample                                                                  *
 *                                                                           *
 * Some people write 16-bit RGB TGA files. The spec would probably allow     *
 * 27-bit RGB too, for what it's worth, but I won't fix that unless someone  *
 * actually provides an existence proof.                                     *
\*---------------------------------------------------------------------------*/
static void upsample(unsigned char *dest, unsigned char *src, unsigned width, unsigned bytes, unsigned char alphaBits)
{
 ...
 * bgr2rgb                                                                   *
\*---------------------------------------------------------------------------*/
static void bgr2rgb(unsigned char *dest, unsigned char *src, unsigned width, unsigned bytes, unsigned alpha)
{
  if (alpha)
  {
    for (unsigned x = 0; x < width; x++)
 ...
 * read_line                                                                 *
\*---------------------------------------------------------------------------*/
static void read_line (FILE *fp, unsigned char *row, unsigned char *buffer, tga_info *info)
{
 ...
 * ReadImage                                                                 *
\*---------------------------------------------------------------------------*/
static Image *ReadImage(FILE *fp, tga_info *info)
{
  Image *image;
